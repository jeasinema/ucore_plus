...
#define MAX_USB_KBD 255

static struct usb_device *list_kbd[MAX_USB_KBD];
static int list_kbd_count;
...
/* Device name */
//#define DEVNAME			"usbkbd\0"
const char DEVNAME[] = "usbkbd";
...
static uint32_t usb_kbd_service_key(struct usb_device *dev, int i, int up)
{
	...
	if ((old[i] > 3) &&
	    (memscan((void*)(new + 2), old[i], USB_KBD_BOOT_REPORT_SIZE - 2) ==
...
int usb_get_keyboard() {
    ...
    // first to get an exist keyboard
    if (list_kbd_count > 0) {
        dev = list_kbd[list_kbd_count-1];
    } else {
        // then try to probe a new keyboard
        uint32_t i = 0;
        for (i = 0; i < USB_MAX_DEVICE; ++i) {
            ...
            // found keyboard
            debug("New Keyboard is found\n");
            list_kbd[list_kbd_count++] = dev;
            break;
        }
    }
    ...
    debug("Found a keyboard\n");
#ifdef DEBUG
    usb_display_desc(dev);
	usb_display_config(dev);
#endif    
    return list_kbd_count-1; 

fail:
    debug("keyboard not found\n");
    return -ENODEV;    
}
...
{
	//struct stdio_dev *dev;
    if (kbd_id >= list_kbd_count)
        goto fail;

	struct usb_device *usb_kbd_dev = list_kbd[kbd_id];
	struct usb_kbd_pdata *data;
...
static int probe_usb_keyboard(struct usb_device *dev)
{
	...
	int error;
	debug("%s probe kbd, devnum:%d\n", __func__, dev->devnum);

	/* Try probing the keyboard */
	if (usb_kbd_probe_dev(dev, 0) != 1) {
		debug("%s probe kbd failed, ENOENT, devnum:%d\n", __func__, dev->devnum);
        return -ENOENT;
    }
    ...
    //debug("usb_kdb_dev name:%s\n", usb_kbd_dev.name);
	//usb_kbd_dev.flags =  DEV_FLAGS_INPUT;
	//usb_kbd_dev.getc = usb_kbd_getc;
	//usb_kbd_dev.tstc = usb_kbd_testc;
	//usb_kbd_dev.priv = (void *)dev;
	//TODO: bypass bypss stdio
    //error = stdio_register(&usb_kbd_dev);
	//if (error) {
	//	debug("%s stdio register failed, devnum:%d\n", __func__, dev->devnum);
    //    return error;
    //}

    //TODO: bypass getenv
	//stdinname = getenv("stdin");
    //debug("%s current stdinname:%s\n", __func__, stdinname);
...
//#else
    ...
    //TODO: fix by jeasinema@20170510, strcmp seems to not working properly
	//if (strcmp(stdinname, DEVNAME)) {
	//	//debug("%s, current stdinnam:%s is already:%s\n", stdinname, DEVNAME);
	//	debug("%s, current stdinnam:%s is already usbkbd\n", __func__, stdinname);
    //    return 1;
    //}
    ...
    //TODO: bypass console_assign
    //debug("before console_assing\n");
	//error = console_assign(stdin, DEVNAME);
	//if (error) {
    //    debug("%s console_assign failed, devnum:%d\n", __func__, dev->devnum);
	//	return error;
    //}
//#endif

    debug("%s probe kbd success, devnum:%d\n", __func__, dev->devnum);
	return 0;
}
...
int drv_usb_kbd_init(void)
{
	...
	for (i = 0; i < USB_MAX_DEVICE; i++) {
		...
		if (!error) {
			debug("%s probe keyboard success, deviceno:%d\n", __func__, i);
            return 1;
        }
		if (error && error != -ENOENT) {
			debug("%s probe keyboard  failed, deviceno:%d errno:%d\n", __func__, i, error);
            return error;
        }
	}

	/* No USB Keyboard found */
	debug("%s probe keyboard failed, no device, errno:%d\n", __func__, error);
	return -1;
}
...
//#endif
	...
	return 0;
err:
	debug("%s: warning, ret=%d", __func__, ret);
	return ret;
}
